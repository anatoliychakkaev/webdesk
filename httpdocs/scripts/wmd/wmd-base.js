...
Attacklab.wmdBase = function () {
	...
	util.addEvent = function (element, event, callback) {
	...
	util.prompt = function (_41,_42,_43) {
		...
		util.removeEvent(_3.body,"keydown",_48);
		var _4d = _47.value;
		if(_4c) {
		_4d = null;
		}
		...
		util.addEvent(_3.body,"keydown",_48);
		_45.style.top = "50%";
		_45.style.left = "50%";
	...
	util.escapeUnderscores = function (_5f) {
		_5f = _5f.replace(/(\S)(_+)(\S)/g,function (util0,util1,util2,util3) {
		util2 = util2.replace(/_/g,"&#95;");
	...
	position.getPageSize = function () {
		...
		util4 = _3.body.scrollWidth;
		util5 = _1.innerHeight+_1.scrollMaxY;
		}else{
		if(_3.body.scrollHeight>_3.body.offsetHeight) {
		util4 = _3.body.scrollWidth;
		util5 = _3.body.scrollHeight;
		}else{
		util4 = _3.body.offsetWidth;
		util5 = _3.body.offsetHeight;
		}
		}
		var util8,util9;
		if(self.innerHeight) {
		util8 = self.innerWidth;
		util9 = self.innerHeight;
		}else{
		if(_3.documentElement&&_3.documentElement.clientHeight) {
		util8 = _3.documentElement.clientWidth;
		util9 = _3.documentElement.clientHeight;
		}else{
		if(_3.body) {
		util8 = _3.body.clientWidth;
		util9 = _3.body.clientHeight;
		}
		}
		}
		if(util5<util9) {
		...
		var utila = [util6,util7,util8,util9];
		return utila;
	};
	position.getPixelVal = function (utilb) {
		if(utilb&&/^(-?\d+(\.\d*)?)px$/.test(utilb)) {
		return _4.$1;
		}
		return undefined;
	};
	position.getTop = function (utilc,utild) {
		var utile = utilc.offsetTop;
		if(!utild) {
		while(utilc = utilc.offsetParent) {
		utile+ = utilc.offsetTop;
		}
		}
		return utile;
	};
	position.setTop = function (utilf,position0,position1) {
		var position2 = position.getPixelVal(utilf.style.top);
		if(position2 == undefined) {
		utilf.style.top = position0+"px";
		position2 = position0;
		}
		var position3 = position.getTop(utilf,position1)-position2;
		utilf.style.top = (position0-position3)+"px";
	};
	position.getLeft = function (position4,position5) {
		var position6 = position4.offsetLeft;
		if(!position5) {
		while(position4 = position4.offsetParent) {
		position6+ = position4.offsetLeft;
		}
		}
		return position6;
	};
	position.setLeft = function (position7,position8,position9) {
		var positiona = position.getPixelVal(position7.style.left);
		if(positiona == undefined) {
		position7.style.left = position8+"px";
		positiona = position8;
		}
		var positionb = position.getLeft(position7,position9)-positiona;
		position7.style.left = (position8-positionb)+"px";
	};
	position.getHeight = function (positionc) {
		var positiond = positionc.offsetHeight;
		if(!positiond) {
		positiond = positionc.scrollHeight;
		}
		return positiond;
	};
	position.setHeight = function (positione,positionf) {
		var _80 = position.getPixelVal(positione.style.height);
		if(_80 == undefined) {
		positione.style.height = positionf+"px";
		_80 = positionf;
		}
		var _81 = position.getHeight(positione)-_80;
		if(_81>positionf) {
			_81 = positionf;
		}
		positione.style.height = (positionf-_81)+"px";
	};
	...
	_2.inputPoller = function (element, callback, poll_interval) {
		...
		var poller = function () {
			...
			//if (util.getStyle(element,"display") = =  "none") {
			//	return;
			//}
			if (poll_object.tick()) {
				callback();
	...
	_2.editor = function (_b0,_b1) {
		...
		_e1();
		util.skin(_b8,_2.basePath+"images/bg.png",_b2,_b3);
	...
	_2.previewManager = function (_169) {
		...
		var smart_change = function (element, callback) {
			util.addEvent(element, "input", callback);
			element.onpaste = callback;
			element.ondrop = callback;
			util.addEvent(_1, "keypress", callback);
			util.addEvent(element, "keypress", callback);
			util.addEvent(element, "keydown", callback);
