...
$.fn.tabs = function(initial, settings) {
    ...
    return this.each(function() {
        ...
        //BUG $('li', nav).removeClass(settings.selectedClass).eq(settings.initial).addClass(settings.selectedClass); // we need to remove classes eventually if hash takes precedence over class
        // trigger load of initial tab
        //BUG tabs.eq(settings.initial).trigger('loadRemoteTab').end();
        ...
        return function(tab) {
            return this.each(function() {
                ...
                if (!tab || typeof tab == 'number') {
                   // BUG a = $('li a', nav).eq((tab && tab > 0 && tab - 1 || 0)); // fall back to 0
                } else if (typeof tab == 'string') {
                    // BUG a = $('li a[@href$="#' + tab + '"]', nav);
                }
                a.trigger(tabEvent);
            });
        };
