...
(function() {
	...
	add("asFormat",function(fmt){
		// {Y}-{m}-{d} {H}:{M}:{S} => 2008-01-29 16:12:40
		if(!fmt)fmt = '{d} {Ba} {Y} в {H}:{M}';
		var d = {
			A: this.getDayName(), 
			b: this.getMonthName(true), // краткое название
			B: this.getMonthName(false), // полное название
			Ba:this.getMonthName(false,true), // полное название в родительном падеже
			m: x(this.getMonth()+1), 
			d: x(this.getDate()),
			H: x(this.getHours()),
			M: x(this.getMinutes()),
		...
		return fmt;//this.getDayName()+', '+this.getDate()+' '+this.getMonthName(true)+' '+this.getFullYear()+' '+this.getHours()+':'+this.getMinutes();
	});
	/**
